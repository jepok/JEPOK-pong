...
function Ball(xpos,ypos,bsize){
  ...
  this.move = function() {
    ...
    function Paddle(xpos,ypos,plength,pwidth,speed,paddleColor){
     ...
     // the paddle.update function controls the computer paddles movement and inherits
     // from the paddle constructor.  In the future a player will be able to pick
     // which side(paddle) to play as well as an option for two players...the game
     // will be able to set up each paddle depending on the paddle.move and paddle.update methods
    ...
    // Step function calls render() and checks to see if game is still going before
    // continuing on with updating animation frames moving the ball and paddles
    var Step = function () {
      render();
      //check to see if game is still going before continuing
      if(gameStart==true){
        animate(Step);
        computerPaddle.update();
        nball.move();
    ...
    // Serve function resets the ball position and ySpeed at the beginning of each
    // new point.  it defines a random yspeed(the xspeed is determined in the balls
    // move method which handles collision detection)
    var Serve = function() {
      nball.xpos = 190;
      nball.ypos = 200;
    ...
    // GameControl function handles starting the game checking the score for a winnerIs
    // and ending the game
